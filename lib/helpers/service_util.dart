  ...
  static Response<dynamic> call({required Response<dynamic> execute}) {
    ...
    try {
      ...
      if (status.connectionError) {
        Get.snackbar("Terjadi masalah koneksi!",
        ...
        Get.snackbar("Server aplikasi terjadi masalah!",
            "Tunggu beberapa saat, dan coba masuk kembali aplikasi");
        log.e("Error server at service: ${conn.request?.url}",
            error: conn.body, stackTrace: StackTrace.current);
      } else if (status.isNotFound) {
        Get.snackbar("Service ini tidak diketahui!",
            "Tunggu beberapa saat, dan coba masuk kembali aplikasi");
        log.e("Error not found at service: ${conn.request?.url}",
            error: conn.body, stackTrace: StackTrace.current);
      } else if (status.isForbidden) {
        Get.snackbar("Terjadi masalah yang tidak terduga",
            "Tunggu beberapa saat, dan coba masuk kembali aplikasi");
        log.e("Error forbidden at service: ${conn.request?.url}",
            error: conn.body, stackTrace: StackTrace.current);
      } else if (status.isUnauthorized) {
        Get.offAll(() => const LoginScreen());
        GetStorage().remove('token');
        Get.snackbar("Sesi kamu telah habis!", "Silahkan login kembali");
        log.i("you are un-authorized at service: ${conn.request?.url}",
            error: conn.body, stackTrace: StackTrace.current);
      } else {
        log.i(
